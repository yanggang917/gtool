    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        changePage: function (ctype)
        {
            ...
            switch (ctype)
            {
                case 'first': if (p.page == 0) return; p.newPage = 0; break; /** 修改:将默认页变为0*/
                case 'prev': if (p.page == 0) return; if (p.page > 0) p.newPage = parseInt(p.page) - 1; break;
                case 'next': if (p.page >= p.pageCount-1) return; p.newPage = parseInt(p.page) + 1; break;
                case 'last': if (p.page >= p.pageCount-1) return; p.newPage = p.pageCount-1; break;
                case 'input':
            ...
            if (p.newPage >= (p.pageCount-1))//已修改成pageCount-1,==修改为>=
            {
        ...
        _showData: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                /* 自己修改的代码逻辑：当有Total属性时，使用Total，而不使用查询出来的数据总数*/
               /* if(p.data.Total != undefined){
                	p.total = p.data[p.record];
                }else{                		
                	p.total = data.length;
                }*/
                p.page = p.newPage;
                if (!p.total) p.total = 0;
                if (!p.page) p.page = 0;//已修改将page由1成为0
                p.pageCount = Math.ceil(p.total / p.pageSize);
        ...
        {
            var g = this, p = this.options;
            $('.pcontrol input', g.toolbar).val((p.page+1));//已修改：改为page+1
            if (!p.pageCount) p.pageCount = 1;
            ...
            stat = stat.replace(/{to}/, (r2+1));//已修改成r2+1
            ...
            if (p.page == p.pageCount-1)//已修改成pageCount-1
            {
                $(".l-bar-btnlast span", g.toolbar).addClass("l-disabled");
                $(".l-bar-btnnext span", g.toolbar).addClass("l-disabled");
            }
